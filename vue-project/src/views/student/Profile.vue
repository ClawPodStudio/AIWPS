<template>
  <div class="profile">
    <el-row :gutter="20">
      <el-col :span="8"><el-card class="profile-card">
        <div style="text-align:center;padding:20px">
          <el-avatar :size="100">{{ userStore.userInfo.name?.charAt(0)||'学' }}</el-avatar>
          <h3 style="margin:16px 0 8px">{{ userStore.userInfo.name||'学生' }}</h3><p style="color:#999">学号：{{ userStore.userInfo.username }}</p>
        </div>
        <el-divider />
        <div class="info-list">
          <div class="info-item"><span class="label">所属机构：</span><span>{{ userStore.userInfo.orgName||'-' }}</span></div>
          <div class="info-item"><span class="label">班级：</span><span>{{ userStore.userInfo.className||'-' }}</span></div>
          <div class="info-item"><span class="label">年级：</span><span>{{ userStore.userInfo.gradeName||'-' }}</span></div>
        </div>
      </el-card></el-col>
...
const profileForm = reactive({ name: '', mobile: '', gradeId: '' })
...
onMounted(()=>{ const i=userStore.userInfo; profileForm.name=i.name||''; profileForm.mobile=i.mobile||''; profileForm.gradeId=i.gradeId||'' })
</script>

<style scoped>
.profile-card{text-align:center}.info-list{text-align:left}.info-item{display:flex;padding:12px 0;border-bottom:1px solid #f5f7fa}.info-item:last-child{border-bottom:none}.info-item .label{width:100px;color:#999}
</style>
